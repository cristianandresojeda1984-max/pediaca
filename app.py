...
def producto_nuevo():
    ...
    execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (restaurante["id"], categoria_id, nombre, descripcion, precio, disponible))
...
def whatsapp_link():
    """
    Recibe el carrito y genera el link de WhatsApp para el local.
    Body JSON: { restaurante_id, items: [{nombre, cantidad, precio}], tipo_entrega, direccion, notas }
    """
    ...
    # Armar el mensaje
    lineas = [f"🛵 *Pedido para {restaurante['nombre_local']}*\n"]
    total  = 0
    for item in items:
        subtotal = item["cantidad"] * item["precio"]
        total   += subtotal
        lineas.append(
            f"• {item['cantidad']}x {item['nombre']} — ${subtotal:,.0f}".replace(",", ".")
        )
    ...
    mensaje = "\n".join(lineas)
    numero  = restaurante["whatsapp"].replace("+", "").replace("-", "").replace(" ", "")
    ...
    # Registrar pedido (anónimo si no hay sesión)
    cliente_id             = session.get("user_id")
    nombre_cliente_anonimo = data.get("nombre") if not cliente_id else None
    tel_anonimo            = data.get("telefono") if not cliente_id else None
    ...
    pedido_id = execute("""
        ...
        VALUES (?,?,?,?,?,?,?,?,1)
    """, (restaurante_id, cliente_id, nombre_cliente_anonimo, tel_anonimo,
